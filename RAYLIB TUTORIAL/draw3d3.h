#include "libs.h"
    ...
    {
        Draw3DBillboard(camera2,texture1,
    ...
    if (MouseRayCollision.hit && !GridSnap)
    {
        Draw3DBillboard(camera2,texture1,{MouseRayCollision.point.x,MouseRayCollision.point.y,MouseRayCollision.point.z},0.1f,GRAY);
    }
    ...
    {   
        if (GroupSelect > -1)
        {
            if (int(vertices1[i].w) == GroupSelect)
            {
                if (vec2.size() > 0)
                {
                    for (int j = 0; j < int(vec2.size()); j++)
                    {
                        ...
                        {
                            Draw3DBillboard(camera2,texture1,{vertices1.at(i).x,vertices1.at(i).y,vertices1.at(i).z},0.1f,{255,255,255,255});
                            boollol = false;
                        }
                    }
                    if (boollol)
                    {
                        Draw3DBillboard(camera2,texture1,{vertices1.at(i).x,vertices1.at(i).y,vertices1.at(i).z},0.1f,RED);
                    }
                    boollol = true;
                }
                else
            ...
            if (int(vertices1[i].w) != GroupSelect)
            {
                Draw3DBillboard(camera2,texture1,{vertices1.at(i).x,vertices1.at(i).y,vertices1.at(i).z},0.1f,DARKGRAY);
            }
        }
        if (GroupSelect == -1)
        {
